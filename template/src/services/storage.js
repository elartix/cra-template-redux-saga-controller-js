...
class Storage {
   ...
   * @param {String} name
   * @return {{path: *, set: (function(*=): void), get: (function(): any), remove: (function(*=): void), update: (function(*=): void),}}
   */
  bindToPath = name => ({
    name,
    get: () => this.get(name),
...
// NOTE generate TAB id for feature debug logs
const TabIDStore = session.bindToPath('uid');
const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
const uid = template => String(template).replace(/X/g, () => chars.charAt(Math.floor(Math.random() * chars.length)));
const tabId = TabIDStore.get();
!tabId && TabIDStore.set(uid('XXX-XXX'));
export { TabIDStore };
